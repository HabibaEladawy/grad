...
extension ThemeModelHelper on ThemeModel {
  ...
  double getFontSize(String sizeKey) {
    final size = this.typographyArabic?.typeface?.size;
    switch (sizeKey) {
      case "SM":
        return double.tryParse(size?.sM?.value ?? '14') ?? 14;
      case "MD":
        return double.tryParse(size?.mD?.value ?? '16') ?? 16;
      case "LG":
        return double.tryParse(size?.lG?.value ?? '18') ?? 18;
      case "XL":
        return double.tryParse(size?.xL?.value ?? '20') ?? 20;
      case "2XL":
        return double.tryParse(size?.d2XL?.value ?? '24') ?? 24;
      case "3XL":
        return double.tryParse(size?.d3XL?.value ?? '28') ?? 28;
      case "4XL":
        return double.tryParse(size?.d4XL?.value ?? '32') ?? 32;
      case "5XL":
        return double.tryParse(size?.d5XL?.value ?? '36') ?? 36;
      default:
        return 16;
    }
  }
